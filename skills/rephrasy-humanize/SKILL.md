---
name: rephrasy-humanize
description: Humanize AI-generated text via the Rephrasy Humanizer API (cloud service, costs credits per call). Use when (1) the user asks to humanize text "with rephrasy" or "via the API", (2) text needs to pass AI detectors and the local StealthHumanizer is not wanted, (3) a slash command like /rephrasy-humanize delegates to it. Defaults to model v3 with style professional. Output goes next to input as foo.txt -> foo_rephrasy.txt. Supports verify (re-detect) and iterate-until-threshold. Requires REPHRASY_API_KEY (already in env / ~/.config/zsh/secrets.zsh).
...
python3 "$HUMANIZE" --text "literal text"        # string -> stdout
...
|---|---|---|
| v3 + `professional` (our default) | 99.87 | 0.002178 |
...
- **`professional` style costs ~3x** the no-style call. Keep it as the default (Josh's preference, and GPTZero — not this detector — is the usual real target), but mention the trade-off if the user is iterating on cost.
- A threshold of **≤ 40** is a realistic multi-pass target for normal-length documents; chasing ≤ 10 usually requires a casual register. For short snippets, judge by trend across passes rather than absolute score, and don't burn more than 3 passes. See the `rephrasy-detect` skill for the full calibration table.

## API response quirks (verified live)

- `costs` is a **plain number** (credits), not the `{"total": ...}` object the marketing page shows. The script handles both displays.
...
## Relationship to the local StealthHumanizer

Josh also has a **local** humanizer (`stealth-humanize` skill, `/humanize` command) that runs through Claude/Codex CLIs at no API cost. This skill is the **cloud Rephrasy** path — use it when the user names Rephrasy, wants the v3 model specifically, or the local tool's output isn't passing detectors. Outputs use the `_rephrasy` suffix precisely so the two never clobber each other (`foo_humanized.txt` vs `foo_rephrasy.txt`).
